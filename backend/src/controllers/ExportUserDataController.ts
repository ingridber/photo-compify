...
export async function exportUserData(req: Request, res: Response) {
  try {
    ...
    // Fetch competitions where user's submissions exist
    const competitionsParticipated = await Competition.find({
      submissions: { $in: submissions.map((s) => s._id) },
    }).lean();

    // Map participated competitions into safe format
    const safeCompetitionsParticipated = competitionsParticipated.map((c) => ({
      id: c._id, // Competition ID
      title: c.title, // Competition title
      phase: c.phase, // Current phase/status
    }));
    ...
    return res.json({
      ...
      competitionsParticipated: safeCompetitionsParticipated, // Competitions user participated in
    });
  } catch (err) {
